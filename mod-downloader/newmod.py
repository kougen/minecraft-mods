...
class Mod():
    ...
    def __init__(self, m_id: str, read_path: str, write_path: str, category: str):
        ...
        def raise_read(key: str):
            value = self.read_service.read(key)
            if value is None:
                raise KeyError(f"{key} not found for {self.mod_id}")
            return value
        self.mod_id = m_id
        self.read_service = JsonService(read_path)
        self.write_service = JsonService(write_path)
        self.category = category
        self.domain = raise_read(f'{self.mod_id}/domain')
        self.file_name = raise_read(f'{self.mod_id}/filename')
        self.depend_on_str = raise_read(f'{self.mod_id}/depend_on')
        self.state = raise_read(f'{self.mod_id}/state')
